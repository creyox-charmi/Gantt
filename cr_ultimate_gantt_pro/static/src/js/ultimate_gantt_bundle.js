...
export class UltimateGanttModel extends Model {
    ...
    async load(params) {
        ...
        const allResources = await this.orm.searchRead("resource.resource", [], ["name", "resource_type", "cost_rate", "city", "max_units", "accrue_at", "material_label"]);
        let assignments = [];
        if (tasks.length > 0) {
            assignments = await this.orm.searchRead("project.task.resource.assignment", [["task_id", "in", tasks.map(t => t.id)]], ["task_id", "resource_id", "units", "cost"]);
        }
        let calDomain = [];
        ...
        const rMap = {}; this.allResources.forEach(r => rMap[r.id] = r);
        let taskAssignments = {};
        assignments.forEach(a => {
            const tid = Array.isArray(a.task_id) ? a.task_id[0] : a.task_id;
            const rid = Array.isArray(a.resource_id) ? a.resource_id[0] : a.resource_id;
            if (!taskAssignments[tid]) taskAssignments[tid] = [];
            taskAssignments[tid].push({ id: a.id, resource_id: rid, units: a.units, cost: a.cost, resource: rMap[rid] });
        });
        ...
        this.data = res.map((p, idx) => {
            ...
            let tks = tasks.filter(t => t.project_id && t.project_id[0] === p.id).map(t => {
                ...
                const taskResources = taskAssignments[t.id] || [];
                const isMs = t.is_milestone || (s === e);
                return { ...t, is_milestone: isMs, planned_date_begin: s, date_deadline: e, real_duration: diff.days > 0 ? `${Math.round(diff.days)}d ${Math.round(diff.hours || 0)}h` : `${Math.round(diff.hours || 0)}h`, assignee_names: assignees.map(u => u.name).join(', '), assignees: assignees, _resources: taskResources };
            });
                        ...
                        <t t-foreach="this.visibleProjects" t-as="p" t-key="p.id">
                            ...
                            <t t-foreach="p.visibleTasks" t-as="t" t-key="t.id">
                                 <div class="o_ug_row" t-att-style="'height: '+state.config.gantt_row_height+'px; width: max-content; min-width: 100%; ' + (state.editingCell &amp;&amp; state.editingCell.id === t.id ? 'overflow: visible !important; z-index: 999;' : '')" t-on-mouseenter="() => this.onPEnter(t)" t-on-mouseleave="this.onPLeave" t-att-class="{ 'o_ug_row_hover': state.hId === t.id, 'text-decoration-line-through text-muted opacity-50': t.inactive }" t-on-contextmenu.prevent="(ev) => this.onContextMenu(ev, t)">
                                     ...
                                     <div t-attf-class="o_ug_sidebar_col flex-shrink-0 {{ t.inactive ? 'text-muted' : 'text-dark' }}" t-attf-style="padding-left: {{ 24 + (t.depth+1)*28 }}px !important; font-weight: 600; width: {{state.colWidths.name}}px; position: relative;" t-on-dblclick="() => this.startCellEdit(t, 'name')">
                                     ...
                                     <div t-if="state.config.gantt_show_assignees" class="o_ug_sidebar_col opacity-75 fw-bold" t-attf-style="width: {{state.colWidths.res}}px; font-size:10px; cursor: pointer; position: relative; overflow: visible !important;" t-on-dblclick="() => this.startCellEdit(t, 'resource_assignment_ids')">
                                         <t t-if="state.editingCell &amp;&amp; state.editingCell.id === t.id &amp;&amp; state.editingCell.field === 'resource_assignment_ids'">
                                             ...
                                             <div class="o_ug_dep_dropdown d-flex flex-column bg-white shadow-sm border rounded" style="position: absolute; top: 100%; left: 0; min-width: 320px; z-index: 1050; padding: 0;">
                                                 <div class="d-flex border-bottom bg-light">
                                                     <div t-attf-class="p-2 border-end fw-bold text-center {{ state.inlineResourceTab === 'work' || !state.inlineResourceTab ? 'text-dark' : 'text-muted' }}" style="font-size: 11px; cursor: pointer; flex: 1;" t-on-click="() => state.inlineResourceTab = 'work'"><i class="fa fa-users me-1"/>Work</div>
                                                     <div t-attf-class="p-2 border-end text-center {{ state.inlineResourceTab === 'material' ? 'text-dark fw-bold' : 'text-muted' }}" style="font-size: 11px; cursor: pointer; flex: 1;" t-on-click="() => state.inlineResourceTab = 'material'"><i class="fa fa-cubes me-1"/>Material</div>
                                                     <div t-attf-class="p-2 text-center {{ state.inlineResourceTab === 'cost' ? 'text-dark fw-bold' : 'text-muted' }}" style="font-size: 11px; cursor: pointer; flex: 1;" t-on-click="() => state.inlineResourceTab = 'cost'"><i class="fa fa-money me-1"/>Cost</div>
                                                 </div>
                                                 <div t-if="state.inlineResourceTab === 'work' || !state.inlineResourceTab">
                                                     <div class="d-flex align-items-center bg-light border-bottom p-1" style="font-size: 10px; font-weight: bold;">
                                                         <div style="width: 24px; text-align: center;">-</div>
                                                         <div style="flex: 1; padding-left: 4px;">Name</div>
                                                         <div style="width: 60px; text-align: center;">Units</div>
                                                         <div style="width: 60px; text-align: center;">Rate table</div>
                                                     </div>
                                                 </div>
                                                 <div t-if="state.inlineResourceTab === 'material'" class="p-3 text-center text-muted" style="font-size: 11px;">
                                                     <i class="fa fa-cubes mb-2 d-block" style="font-size: 24px; opacity: 0.5;"></i>
                                                     No material resources found.
                                                 </div>
                                                 <div t-if="state.inlineResourceTab === 'cost'" class="p-3 text-center text-muted" style="font-size: 11px;">
                                                     <i class="fa fa-money mb-2 d-block" style="font-size: 24px; opacity: 0.5;"></i>
                                                     No cost resources found.
                                                 </div>
                                                 <t t-if="state.inlineResourceTab === 'work' || !state.inlineResourceTab">
                                                     <div style="max-height: 200px; overflow-y: auto;">
                                                         <t t-foreach="this.props.model.allResources" t-as="res" t-key="res.id">
                                                             <t t-set="rVal" t-value="state.editingCell.val.find(r => r.resource_id === res.id)"/>
                                                             <div class="d-flex align-items-center border-bottom py-1 hover-bg-light" style="font-size: 11px;">
                                                                 <div style="width: 24px; text-align: center;">
                                                                     <input type="checkbox" style="cursor: pointer;" t-att-checked="!!rVal" t-on-change="(ev) => this.toggleInlineResource(res.id, ev.target.checked)"/>
                                                                 </div>
                                                                 <div style="flex: 1; padding-left: 4px; display: flex; align-items: center;">
                                                                     <i class="fa fa-user-circle text-muted fs-5 me-2"/>
                                                                     <span class="text-truncate fw-normal text-dark"><t t-esc="res.name"/></span>
                                                                 </div>
                                                                 <div style="width: 60px; text-align: center;">
                                                                     <input t-if="rVal" type="number" class="form-control form-control-sm border-0 bg-transparent text-end px-1" t-att-value="rVal.units" t-on-change="(ev) => this.updateInlineResource(res.id, 'units', ev.target.value)"/>
                                                                 </div>
                                                                 <div style="width: 60px; text-align: center;">
                                                                     <span t-if="rVal" class="text-muted">Default</span>
                                                                 </div>
                                                             </div>
                                                         </t>
                                                     </div>
                                                     <div class="p-2 bg-light text-center border-top">
                                                         <button class="btn btn-sm btn-primary py-1 px-4 fw-bold me-2" style="font-size: 11px; border-radius: 4px;" t-on-click="this.commitCellEdit">Save</button>
                                                         <button class="btn btn-sm btn-light border py-1 px-3" style="font-size: 11px; border-radius: 4px;" t-on-click="() => { state.editingCell = null; }">Cancel</button>
                                                     </div>
                                                 </t>
                                             </div>
                                         </t>
                                         <t t-if="t._resources &amp;&amp; t._resources.length">
                                             <div class="d-flex align-items-center ms-2" style="padding-left: 8px;">
                                                 <t t-foreach="t._resources" t-as="ra" t-key="ra.id">
                                                     <div class="rounded-circle text-white d-flex align-items-center justify-content-center shadow-sm" 
                                                          t-att-title="ra.resource ? ra.resource.name + (ra.units !== 100 ? ' (' + ra.units + '%)' : '') : ''" 
                                                          style="width: 24px; height: 24px; font-size: 9px; font-weight: 700; border: 2px solid white; margin-left: -8px; background-color: #64748b; z-index: 1;">
                                                         <t t-if="ra.resource">
                                                             <t t-esc="ra.resource.name.substring(0, Math.min(2, ra.resource.name.length)).toUpperCase()"/>
                                                         </t>
                                                     </div>
                                                 </t>
                                             </div>
...
<t t-if="(!t.children || t.children.length === 0) and !t.is_milestone">
                        ...
                        <div class="o_ug_modal" t-on-click.stop="() => { state.openPredDropdownId = null; state.openSuccDropdownId = null; state.openPredTypeDropdownId = null; state.openSuccTypeDropdownId = null; }">
                            ...
                            <div class="o_ug_modal_body">
                                <t t-if="state.editorTab === 'general'">
                                    ...
                                    <div class="row align-items-center mb-3">
                                        <div class="col-3 text-start text-muted" style="font-size: 13px;">Allocated Time</div>
                                        <div class="col-9">
                                    ...
                                    <div class="row align-items-center mb-3">
                                        <div class="col-3 text-start text-muted" style="font-size: 13px;">Color</div>
                                    ...
                                    <div class="d-flex flex-column h-100">
                                        <div class="flex-grow-1" style="max-height: 300px; overflow: visible !important;">
                                             <table class="o_ug_table" style="overflow: visible !important;">
                                                <thead><tr><th>Resource</th><th style="width:100px;">Units (%)</th><th style="width:100px;">Cost</th><th style="width:50px;"></th></tr></thead>
                                                <tbody>
                                                    <t t-foreach="state.editorTask._resources" t-as="ra" t-key="ra_index">
                                                        <tr>
                                                            <td class="d-flex align-items-center gap-2">
                                                                <select class="form-select form-select-sm border-0 bg-transparent" t-on-change="(ev) => this.updateResource(ra_index, ev.target.value)">
                                                                    <t t-foreach="this.props.model.allResources" t-as="ou" t-key="ou.id">
                                                                        <option t-att-value="ou.id" t-att-selected="ou.id === ra.resource_id"><t t-esc="ou.name"/></option>
                                                                    </t>
                                                                </select>
                                                            </td>
                                                            <td><input type="number" class="form-control form-control-sm border-0 bg-transparent text-center" t-att-value="ra.units" t-on-change="(ev) => this.updateResourceUnits(ra_index, ev.target.value)"/></td>
                                                            <td><input type="number" class="form-control form-control-sm border-0 bg-transparent text-center" t-att-value="ra.cost" t-on-change="(ev) => this.updateResourceCost(ra_index, ev.target.value)"/></td>
                                                            <td class="text-center"><i class="fa fa-trash-o text-danger cursor-pointer" t-on-click="() => this.delRes(ra_index)"/></td>
                                                        </tr>
                                                    </t>
                                                </tbody>
                                            </table>
             ...
             <div t-if="state.hoverTask or (state.hoverRollupTasks and state.hoverRollupTasks.length > 0)" class="o_ug_tooltip" t-att-style="'left: '+state.mouseX+'px; top: '+state.mouseY+'px;'">
                ...
                <t t-else="">
                    ...
                    <div class="o_ug_tooltip_row">
                        <span class="o_ug_tooltip_label">Allocated Time:</span>
                        <span class="o_ug_tooltip_val"><t t-esc="state.hoverTask.effort"/> hours</span>
                    </div>
...
`;
    ...
    toggleInlineResource(rId, checked) {
        if (!this.state.editingCell || this.state.editingCell.field !== 'resource_assignment_ids') return;
        if (checked) {
            if (!this.state.editingCell.val.find(r => r.resource_id === rId)) {
                this.state.editingCell.val.push({ resource_id: rId, units: 100, cost: 0 });
            }
        } else {
            this.state.editingCell.val = this.state.editingCell.val.filter(r => r.resource_id !== rId);
        }
    }
    updateInlineResource(rId, field, value) {
        if (!this.state.editingCell || this.state.editingCell.field !== 'resource_assignment_ids') return;
        const res = this.state.editingCell.val.find(r => r.resource_id === rId);
        if (res) res[field] = value;
    }
    startCellEdit(task, field) {
        console.log("Double clicked cell! task:", task.id, "field:", field, "Inline Editing Enabled:", this.state.config.gantt_enable_cell_editing);
        if (!this.state.config.gantt_enable_cell_editing) {
            console.log("Cell editing is disabled in UI settings, ignoring double click.");
            return;
        }
        if (task.id.toString().startsWith('proj_')) {
            console.log("Cannot inline-edit project summary row.");
            return;
        }
        let val = task[field];
        ...
        if (field === 'resource_assignment_ids') {
            val = (task._resources || []).map(r => {
                let rId = Array.isArray(r.resource_id) ? r.resource_id[0] : r.resource_id;
                return { resource_id: rId, units: r.units, cost: r.cost };
            });
            this.state.editingCell = { id: task.id, field, val, original: JSON.parse(JSON.stringify(val)) };
            console.log("Initialized inline resource editor with val:", val);
            return;
        }
    ...
    async commitCellEdit() {
        ...
        if (field === 'resource_assignment_ids') {
            const valStr = val.map(r => `${r.resource_id}-${r.units}-${r.cost}`).sort().join(',');
            const origStr = original.map(r => `${r.resource_id}-${r.units}-${r.cost}`).sort().join(',');
        ...
        if (field === 'depend_on_ids' || field === 'dependent_ids') {
            ...
            writeVals[field] = finalVal;
        } else if (field === 'calendar_id' || field === 'ignore_resource_calendar') {
            if (field === 'calendar_id') {
                finalVal = (val === 'false' || !val) ? false : parseInt(val, 10);
            }
            writeVals[field] = finalVal;
        } else if (field === 'resource_assignment_ids') {
            writeVals[field] = [[5, 0, 0]];
            val.forEach(r => {
                writeVals[field].push([0, 0, { resource_id: parseInt(r.resource_id), units: parseFloat(r.units) || 100, cost: parseFloat(r.cost) || 0 }]);
            });
        } else {
            writeVals[field] = finalVal;
        }
    ...
    async openTaskEditor(t, tab = 'general') {
        ...
        this.state.editorTask = {
            ...t,
            planned_date_begin: t.planned_date_begin ? deserializeDateTime(t.planned_date_begin).toFormat("yyyy-MM-dd'T'HH:mm") : null,
            date_deadline: t.date_deadline ? deserializeDateTime(t.date_deadline).toFormat("yyyy-MM-dd'T'HH:mm") : null,
            constraint_date: t.constraint_date ? deserializeDateTime(t.constraint_date).toFormat("yyyy-MM-dd'T'HH:mm") : null,
            ...
            _resources: t._resources ? t._resources.map(r => ({ ...r })) : [],
    ...
    async saveEditor() {
        ...
        const calId = et.ignore_resource_calendar ? false : toCalId(et.calendar_id);
        const oldCalId = original.ignore_resource_calendar ? false : toCalId(original.calendar_id);

        // --- Calendar-aware date recalculation ---
        const allCals = this.props.model.allCalendars || [];
        const DEFAULT_HOURS = 8;
        
        const parseToUTC = (localStr) => {
            if (!localStr) return false;
            return serializeDateTime(luxon.DateTime.fromFormat(localStr, "yyyy-MM-dd'T'HH:mm"));
        };

        let sStr = parseToUTC(et.planned_date_begin);
        let eStr = parseToUTC(et.date_deadline);
        ...
        const vals = {
            ...
            constraint_date: parseToUTC(et.constraint_date),
        ...
        vals.resource_assignment_ids = [[5, 0, 0]];
        const newResources = [...(et._resources || [])];
        newResources.forEach(r => {
            vals.resource_assignment_ids.push([0, 0, {
        ...
        // Also update standard user_ids if the resource happens to map to a user
        // We'll just pass a command to clear it if empty, or we let the backend handle it.
        // Actually, we can just leave user_ids alone and let project.task.resource.assignment logic manage it on the backend later.
        ...
        if (et.is_project) {
            ...
            delete writeVals.resource_assignment_ids;
        }
    ...
    addRes() {
        const first = this.props.model.allResources.find(u => !this.state.editorTask._resources.some(r => r.resource_id === u.id));
        if (first) {
            this.state.editorTask._resources.push({
                resource_id: first.id,
                units: 100,
                cost: first.cost_rate || 0,
                resource: first
            });
        }
    }
    delRes(idx) {
        this.state.editorTask._resources.splice(idx, 1);
    }
    updateResource(idx, newId) {
        const res = this.props.model.allResources.find(r => r.id === parseInt(newId));
        if (res && this.state.editorTask._resources[idx]) {
            this.state.editorTask._resources[idx].resource_id = res.id;
            this.state.editorTask._resources[idx].resource = res;
            this.state.editorTask._resources[idx].cost = res.cost_rate || 0;
        }
    }
    updateResourceUnits(idx, units) {
        if (this.state.editorTask._resources[idx]) this.state.editorTask._resources[idx].units = parseFloat(units) || 0;
    }
    updateResourceCost(idx, cost) {
        if (this.state.editorTask._resources[idx]) this.state.editorTask._resources[idx].cost = parseFloat(cost) || 0;
    }
    ...
    async onAutoSchedule() {
        ...
        // Advanced Forward Pass Auto-Scheduling with Constraints
        let changed = true; let iterations = 0;
        while (changed && iterations < 100) {
            changed = false; iterations++;
            allTasks.forEach(t => {
                if (t.manually_scheduled) return;
                ...
                activePreds.forEach(pId => {
                    ...
                    if (pred) {
                        // Default to FS if dependencyMap not found
                        const depType = this.dependencyMap ? (this.dependencyMap[`${t.id}_${pId}`] || 'FS') : 'FS';
                        if (depType === 'FS') {
                            if (!maxEnd || pred.e > maxEnd) maxEnd = pred.e;
                        } else if (depType === 'SS') {
                            if (!maxEnd || pred.s > maxEnd) maxEnd = pred.s; // Starts after pred starts
                        } else if (depType === 'FF') {
                            // Finish to Finish: this task must finish after pred finishes.
                            // Handled later or approximated here by setting maxEnd
                        } else if (depType === 'SF') {
                            // Start to Finish: this task must finish after pred starts.
                        }
                    }
                });

                let earliestStart = maxEnd || cur.s;
                let cDate = t.constraint_date ? deserializeDateTime(t.constraint_date) : null;
                let cType = t.constraint_type || 'none';
                ...
                if (cType === 'mso' && cDate) {
                    ...
                    newS = luxon.DateTime.max(earliestStart, cDate);
                } else if (cType === 'snlt' && cDate) {
                    newS = luxon.DateTime.min(earliestStart, cDate);
                } else if (cType === 'asap') {
                    newS = earliestStart;
                } else if (cType === 'none') {
                    if (maxEnd && cur.s < maxEnd) newS = maxEnd;
                }

                if (+newS !== +cur.s) {
                    cur.s = newS;
                    if (!cur.ignore_resource_calendar) {
                ...
                if (cType === 'mfo' && cDate) {
                    newE = cDate;
                } else if (cType === 'fnet' && cDate) {
                    newE = luxon.DateTime.max(cur.e, cDate);
                } else if (cType === 'fnlt' && cDate) {
                    newE = luxon.DateTime.min(cur.e, cDate);
                }

                if (+newE !== +cur.e) {
                    cur.e = newE;
                    cur.s = cur.e.minus({ minutes: cur.dur });
                    changed = true;
                }
            });
        }
    ...
    async _finishDrag(t, os, OE, chain, dx) {
        // Apply constraint for dragged task
        const newBegin = serializeDateTime(os.plus({ days: dx }));
        const newEnd = serializeDateTime(OE.plus({ days: dx }));

        t.planned_date_begin = newBegin;
        t.date_deadline = newEnd;
        t.constraint_type = 'snet';
        t.constraint_date = newBegin;

        // CLEAR DRAG STATE IMMEDIATELY
        this.state.drag = null; this.state.dragOffset = 0; this.state.dragChainIds = [];

        try {
            // Write constraint and dates for the dragged task
            await this.orm.write("project.task", [t.id], {
                planned_date_begin: newBegin,
                date_deadline: newEnd,
                constraint_type: 'snet',
                constraint_date: newBegin
            });

            // Run full auto-schedule pass to cascade constraints to successors
            await this.onAutoSchedule();
        } catch (err) {
