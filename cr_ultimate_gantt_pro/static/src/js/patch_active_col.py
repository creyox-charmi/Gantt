...
new_preds = '''                                <t t-if="state.editorTab === 'predecessors'">
                                    <div class="d-flex flex-column h-100">
                                        <div class="flex-grow-1 overflow-auto bg-light mb-3" style="max-height: 250px;">
                                            <table class="table table-sm table-borderless table-hover mb-0" style="font-size: 13px;">
                                                <thead class="bg-white border-bottom" style="position: sticky; top: 0; z-index: 1;">
                                                    <tr>
                                                        <th class="fw-bold text-dark" style="padding: 10px;">Name</th>
                                                        <th class="fw-bold text-dark border-start" style="padding: 10px; width: 140px;">Type</th>
                                                        <th class="fw-bold text-dark text-end border-start" style="padding: 10px; width: 100px;">Lag</th>
                                                ...
                                                <tbody>
                                                    <t t-foreach="state.editorTask._preds" t-as="pId" t-key="pId">
                                                        <t t-set="pT" t-value="this.props.model.allTasksList.find(x=>x.id===pId)"/>
                                                        <tr style="cursor: pointer; background: white;" class="border-bottom" t-attf-class="{{state.selectedPred === pId ? 'table-primary' : ''}}" t-on-click="() => state.selectedPred = pId">
                                                            <td class="p-0 border-end" style="vertical-align: middle;">
                                                                <select class="form-select form-select-sm border-0 shadow-none text-dark bg-transparent w-100 h-100" t-on-change="(ev) => this.updatePredecessor(pId, ev.target.value)" style="border-radius: 0; min-height: 36px; cursor: pointer;">
                                                                    <t t-foreach="this.props.model.allTasksList" t-as="ot" t-key="ot.id">
                                                                        <option t-att-value="ot.id" t-att-selected="ot.id === pId"><t t-esc="ot.name"/></option>
                                                    ...
                                                    <tr t-if="!state.editorTask._preds || state.editorTask._preds.length === 0">
                                                        <td colspan="4" class="text-center text-muted py-4 bg-white border-bottom">No predecessors defined</td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                        </div>
                                        <div class="d-flex align-items-center gap-3 mt-1 ps-2">
                                            <i class="fa fa-plus fs-5 fw-bold text-dark cursor-pointer" t-on-click="this.addPred"/>
                                            <i class="fa fa-trash fs-5 cursor-pointer" t-attf-style="color: {{state.selectedPred ? '#5d4037' : '#ccc'}};" t-on-click="() => { if(state.selectedPred) { this.delPred(state.selectedPred); state.selectedPred = null; } }"/>
                                        </div>
                                    </div>
                                </t>'''

# Successors Tab Replacement
...
new_succs = '''                                <t t-if="state.editorTab === 'successors'">
                                    <div class="d-flex flex-column h-100">
                                        <div class="flex-grow-1 overflow-auto bg-light mb-3" style="max-height: 250px;">
                                            <table class="table table-sm table-borderless table-hover mb-0" style="font-size: 13px;">
                                                ...
                                                <tbody>
                                                    <t t-set="succs" t-value="this.getTaskSuccessors(state.editorTask.id)"/>
                                                    <t t-foreach="succs" t-as="s" t-key="s.id">
                                                        <tr style="cursor: pointer; background: white;" class="border-bottom" t-attf-class="{{state.selectedSucc === s.id ? 'table-primary' : ''}}" t-on-click="() => state.selectedSucc = s.id">
                                                            <td class="p-0 border-end" style="vertical-align: middle;">
                                                                <select disabled="1" class="form-select form-select-sm border-0 shadow-none text-dark bg-transparent w-100 h-100" style="border-radius: 0; min-height: 36px; cursor: pointer; opacity: 1;">
                                                                    <option selected="1"><t t-esc="s.name"/></option>
                                                                </select>
                                        ...
                                        <div class="d-flex align-items-center gap-3 mt-1 ps-2">
                                            <i class="fa fa-plus fs-5 fw-bold text-dark cursor-pointer"/>
                                            <i class="fa fa-trash fs-5 cursor-pointer" t-attf-style="color: {{state.selectedSucc ? '#5d4037' : '#ccc'}};"/>
                                        </div>
                                    </div>
                                </t>'''
...
print("Added Active column and styling to match Bryntum!")
